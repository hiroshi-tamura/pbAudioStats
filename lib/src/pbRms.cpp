...
RMSMeter::Result RMSMeter::measure(const AudioData& audio, double window_ms) {
    ...
    // Per-sample IIR (1st order) is inherently serial; SIMD parallelization
    // changes the recurrence and produces incorrect smoothed values, so we
    // process samples scalarly and track min/max per frame.
